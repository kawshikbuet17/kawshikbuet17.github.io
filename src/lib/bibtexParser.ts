...
export function parseBibTeX(bibtexContent: string, locale?: string): Publication[] {
  ...
  return entries.map((entry: { entryType: string; citationKey: string; entryTags: Record<string, string> }, index: number) => {
    ...
    // Parse status field
    const validStatuses: PublicationStatus[] = ['in-progress', 'preprint', 'submitted', 'under-review', 'accepted', 'published'];
    const status: PublicationStatus = validStatuses.includes(tags.status as PublicationStatus) ? (tags.status as PublicationStatus) : 'published';
    ...
    const publication: Publication = {
      ...
      period: cleanBibTeXString(tags.period),
      selected,
      preview,
